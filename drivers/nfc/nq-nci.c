...
#ifdef CONFIG_MACH_XIAOMI_MSM8998
static struct wakeup_source fieldon_wakesrc;
#endif
struct nqx_dev {
					...
					size_t count, loff_t *offset)
{
	...
	if (((tmp[0] & 0xff) == 0x61) && ((tmp[1] & 0xff) == 0x07) && ((tmp[2] & 0xff) == 0x01)) {
		__pm_wakeup_event(&fieldon_wakesrc, 3*1000);
	}
#endif
#ifdef NFC_KERNEL_BU
...
#ifdef CONFIG_MACH_XIAOMI_MSM8998
	wakeup_source_init(&fieldon_wakesrc, "nfc_locker");
#endif
...
#ifdef CONFIG_MACH_XIAOMI_MSM8998
	wakeup_source_trash(&fieldon_wakesrc);
#endif
